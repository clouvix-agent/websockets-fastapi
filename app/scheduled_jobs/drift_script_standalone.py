...
#     bucket_name = f"terraform-workspaces-user-{req.user_id}"
#     tf_prefix = f"{req.project_name}_terraform/"

#     # Step 1: Download all .tf files from the folder
#     found_tf_file = False
#     objects = MINIO_CLIENT.list_objects(bucket_name, prefix=tf_prefix, recursive=True)

#     for obj in objects:
#         if obj.object_name.endswith(".tf"):
#             local_tf_path = os.path.join(workspace, os.path.relpath(obj.object_name, tf_prefix))
#             success = download_file(bucket_name, obj.object_name, local_tf_path)
#             if success:
#                 found_tf_file = True

#     if not found_tf_file:
#         raise HTTPException(status_code=404, detail="No .tf files found in MinIO")

#     # Step 2: Download terraform.tfstate
#     tfstate_remote = f"{req.project_name}_terraform/terraform.tfstate"
#     tfstate_local = os.path.join(workspace, "terraform.tfstate")
#     remote_tf_path=f"{req.project_name}_terraform"
...
#         if not download_file(f"terraform-workspaces-user-{req.user_id}", remote_tf_path, local_tf_path):
...
def trigger_drift(req: DriftRequest):
    ...
    try:
        ...
        for obj in objects:
            if obj.object_name.endswith(".tf"):
                local_tf_path = os.path.join(workspace, os.path.relpath(obj.object_name, tf_prefix))
                if "provider.tf" in obj.object_name:
                    provider_tf_path = local_tf_path
                success = download_file(bucket_name, obj.object_name, local_tf_path)
                if success:
                    found_tf_file = True
        ...
        # Step 3: Inject AWS credentials into provider.tf
        if provider_tf_path and os.path.exists(provider_tf_path):
            with open(provider_tf_path, "r") as f:
                original_provider_content = f.read()

            injected_provider_content = []
            inside_provider_block = False

            for line in original_provider_content.splitlines():
                stripped = line.strip()
                if stripped.startswith('provider "aws"'):
                    inside_provider_block = True
                elif inside_provider_block and stripped.startswith("}"):
                    injected_provider_content.append(f'  access_key = "{req.aws_access_key}"')
                    injected_provider_content.append(f'  secret_key = "{req.aws_secret_key}"')
                    inside_provider_block = False

                injected_provider_content.append(line)

            with open(provider_tf_path, "w") as f:
                f.write("\n".join(injected_provider_content))
            print("🔐 AWS credentials injected into provider.tf")

        else:
            raise HTTPException(status_code=400, detail="provider.tf not found")
